...
func CyclicalComplexity(languageInfo types.NodeManagement, code *[]string) []*domain.FunctionData {
	sourceBytes := []byte(strings.Join(*code, "\n"))
	...
	for {
		...
		case len(Stack) <= 0:
			languageInfo.ManageNode(query.CaptureNames(), copyOf.Captures[0], Functions[0], sourceBytes)
		...
		case Stack[len(Stack)-1].IsTargetInRange(copyOf.Captures[0].Node.StartByte(), copyOf.Captures[0].Node.EndByte()):
			...
			languageInfo.ManageNode(query.CaptureNames(), copyOf.Captures[0], Stack[len(Stack)-1], sourceBytes)
